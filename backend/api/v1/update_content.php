...
if ($input['is_admin'] === 0) {
    ...
    // Database connection
    $db = new Database();
    $updateQuery = 'UPDATE news SET title = ? , content = ? , topic = ? WHERE id = ?';
    $rowCount = $db->updateData($updateQuery,array($title,$content, $topic, $content_id));
    if($rowCount) {
        http_response_code(201);
        echo json_encode(['message' => 'Content updated succesfull.']);
    }else {
        http_response_code(500);
        echo json_encode(['message' => 'Content update failed.']);
    }
} else {
